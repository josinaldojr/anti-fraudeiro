...
const resultsPath = __ENV.RESULTS_PATH || 'artifacts/rinha/results.json';

const testData = new SharedArray('test-data', function () {
    return JSON.parse(open('../../.rinha/test/test-data.json')).entries;
});

const statsArr = new SharedArray('test-stats', function () {
    return [JSON.parse(open('../../.rinha/test/test-data.json')).stats];
});
